 ...
 */
public class CryofluxTransducerEffect extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
																					OnHitEffectPlugin,
	...
	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		...
		Iterator<CryofluxTransducerEffect> iter = trails.iterator();
		while (iter.hasNext()) {
		...
		if (weapon.getShip() != null) {
			...
			for (CryofluxTransducerEffect curr : trails) {
				if (curr.proj != null) {
		...
		for (int i = 0; i < numIter; i++) { 
			for (CryofluxTransducerEffect trail : trails) {
...
//								   rampUp, dur, inverted);
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		...
		if (trails == null) {
			trails = new ArrayList<CryofluxTransducerEffect>();
		}
		trails.add(0, trail);
	}
