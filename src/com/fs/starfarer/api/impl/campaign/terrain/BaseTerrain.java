...
//		if (entity.isInCurrentLocation() && entity.getContainingLocation().isHyperspace()) {
...
//			float dx = fleet.getLocation().x - entity.getLocation().x;
//			float dy = fleet.getLocation().y - entity.getLocation().y;
//			float distSq = dx * dx + dy * dy;
//			if (distSq > renderRange) continue;
			
			String cat = getEffectCategory();
			String key = "$terrain_" + cat;
			//String key = cat;
			//if (this instanceof WavefrontTerrainPlugin) continue;
				...
				if (containsPoint(fleet.getLocation(), fleet.getRadius() + getExtraSoundRadius())) {
					...
					if (volumeMult > 0) {
						if (shouldPlayLoopOne()) {
							...
							if (soundId != null) {
								Global.getSector().getCampaignUI().suppressMusic(spec.getMusicSuppression() * suppressionMult);
								Global.getSoundPlayer().playLoop(soundId, fleet, 
											getLoopOnePitch(), getLoopOneVolume() * volumeMult,
						...
						if (shouldPlayLoopTwo()) {
							...
							if (soundId != null) {
								Global.getSector().getCampaignUI().suppressMusic(spec.getMusicSuppression() * suppressionMult);
								Global.getSoundPlayer().playLoop(soundId, fleet, 
											getLoopTwoPitch(), getLoopTwoVolume() * volumeMult,
											fleet.getLocation(), Misc.ZERO);
							}
						}
						if (shouldPlayLoopThree()) {
							...
							if (soundId != null) {
								Global.getSector().getCampaignUI().suppressMusic(spec.getMusicSuppression() * suppressionMult);
								Global.getSoundPlayer().playLoop(soundId, fleet, 
										getLoopThreePitch(), getLoopThreeVolume() * volumeMult,
										fleet.getLocation(), Misc.ZERO);
							}
						}
						if (shouldPlayLoopFour()) {
							...
							if (soundId != null) {
								Global.getSector().getCampaignUI().suppressMusic(spec.getMusicSuppression() * suppressionMult);
								Global.getSoundPlayer().playLoop(soundId, fleet, 
										getLoopFourPitch(), getLoopFourVolume() * volumeMult,
										fleet.getLocation(), Misc.ZERO);
							}
						}
					}
...
//	}
	...
	protected boolean shouldPlayLoopOne() { return getSpec().getLoopOne() != null; }
	protected boolean shouldPlayLoopTwo() { return getSpec().getLoopTwo() != null; }
	protected boolean shouldPlayLoopThree() { return getSpec().getLoopThree() != null; }
	protected boolean shouldPlayLoopFour() { return getSpec().getLoopFour() != null; }
	protected float getLoopOnePitch() { return 1f; }
	protected float getLoopOneVolume() { return 1f; }
	protected float getLoopTwoPitch() { return 1f; }
	protected float getLoopTwoVolume() { return 1f; }
	protected float getLoopThreePitch() { return 1f; }
	protected float getLoopThreeVolume() { return 1f; }
	protected float getLoopFourPitch() { return 1f; }
	protected float getLoopFourVolume() { return 1f; }
