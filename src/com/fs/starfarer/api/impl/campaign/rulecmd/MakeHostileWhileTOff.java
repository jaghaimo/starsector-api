 ...
 * @author Alex Mosolov
 *
 *	MakeHostileWhileTOff <reason> <true or false>
 *
 * Copyright 2015 Fractal Softworks, LLC
 */
public class MakeHostileWhileTOff extends BaseCommandPlugin {
	...
	public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Token> params, Map<String, MemoryAPI> memoryMap) {
		...
		boolean stillSet = Misc.setFlagWithReason(memory, MemFlags.MEMORY_KEY_MAKE_HOSTILE_WHILE_TOFF, reason, value, expire);
		
		if (!stillSet && Global.getSector().getPlayerFleet().isTransponderOn()) {
			CampaignFleetAPI fleet = (CampaignFleetAPI) dialog.getInteractionTarget();
